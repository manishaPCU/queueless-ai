...
const signup = async (req, res) => {
  try {
    const { name, type, email, password, address } = req.body;
    ...
    const org = await Organisation.create({ name, type, email, password: hashed, address });
    ...
    res.status(201).json({ token, org: { id: org._id, name: org.name, qrCode: org.qrCode } });
  } catch (err) {
